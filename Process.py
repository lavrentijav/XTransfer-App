...
def create_large_binary_file(filename, size_in_bytes, info: str = "create file"):
    with tqdm(total=size_in_bytes, unit="B", unit_scale=True, desc=info, ascii=True, colour="green") as pbar:
...
def update_progress_bar(total_size, progress_queue: queue.Queue, info, unit: str = "it", work: bool = True):
    with tqdm(total=total_size, unit=unit, unit_scale=True, desc=info, ascii=True, maxinterval=1,
              mininterval=0.05, colour="green") as pbar:
